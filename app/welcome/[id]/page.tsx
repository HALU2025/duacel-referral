...
export default function OwnerDashboard() {
  ...
  const loadData = async (silent = false) => {
    ...
    const enrichedReferrals = reversedLogs.map((log, index) => {
      ...
      // ★ アクションフィードは「当時のスナップショット」を絶対の事実として計算・表示する
    ...
    // ★ サマリーの金額も、当時のスナップショットから計算された事実（ownerPoints）を元に合計する
    setSummary({
  ...
  const handlePatternSelect = (id: string) => {
    ...
    if (pattern && pattern.ratios) {
      setRatios(pattern.ratios) // UIのみ更新（フィードは更新しない）
    }
  }
  ...
  const policyEditorJSX = (
    <div className="space-y-4 mb-8">
      ...
      <div className="p-4 bg-gray-100 rounded-xl border border-gray-200 flex flex-col gap-2">
        <p className="text-[10px] font-bold text-gray-500">現在設定されているルール</p>
