...
export default function MemberMagicPage() {
  ...
  const loadData = async (silent = false) => {
    ...
    const [refRes, txRes, staffCountRes] = await Promise.all([
      ...
      supabase.from('staffs').select('*').eq('shop_id', staffData.shop_id).eq('is_deleted', false)
    ])

    const allReferrals = refRes.data || []
    const pointLogs = txRes.data || []
    const activeStaffs = staffCountRes.data || []
    ...
    const basePointsDefault = shopData.shop_categories?.reward_points || 0;
    ...
    allReferrals.forEach((r, index) => {
      ...
      const indRatio = r.snapshot_ratio_individual ?? (shopData.ratio_individual ?? 100);
      const teamRatio = r.snapshot_ratio_team ?? (shopData.ratio_team ?? 0);

      // ★ 自分の取り分を計算
      const myIndPart = isMine ? basePoints * (indRatio / 100) : 0;
      const myTeamPart = isMeEligible ? (basePoints * (teamRatio / 100)) / eligibleStaffCount : 0;
      const myEarnedPoints = Math.floor(myIndPart + myTeamPart);

      if (myEarnedPoints > 0) {
        myReferrals.push({ 
          ...r, 
          myTotalPt: isCanceled ? 0 : myEarnedPoints, 
          myIndPart: isCanceled ? 0 : Math.floor(myIndPart),
          myTeamPart: isCanceled ? 0 : Math.floor(myTeamPart),
          totalGenerated: basePoints, // 総発生額
          isMine, 
          hasBonus: isFirstTime && firstBonusEnabled && isMine 
        });

        if (!isCanceled) {
          if (r.status === 'confirmed' || r.status === 'issued' || r.is_staff_rewarded) {
            sTotal += myEarnedPoints;
            sConfirmed += myEarnedPoints; // 簡易的に確定残高として扱う
  ...
  const handleManualLock = () => {
    ...
    setIsUnlocked(false); setActiveTab('stats'); setPin(['', '', '', ''])
  }
  ...
  const handleExchangePay = async () => {
    ...
    // ※実際にはここで point_transactions にマイナス履歴を入れ、外部APIを叩く
  ...
  if (!staff) return <div className="fixed inset-0 flex items-center justify-center bg-white text-gray-500 text-sm">ページが見つかりません。</div>
      ...
      <div className="w-full max-w-md bg-white h-full relative shadow-2xl flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto pb-6 bg-gray-50/30">
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="absolute inset-0 overflow-y-auto pb-24 -webkit-overflow-scrolling-touch">
                               ...
                               {!isPending && !isCanceled && (
                                 <div className="pt-3 mt-3 border-t border-gray-100/80">
                                   <div className="flex justify-between text-[10px] mb-1.5">
                                     <span className="text-gray-500">総発生額</span>
                                   ...
                                   {item.myTeamPart > 0 && (
                                     <div className="flex justify-between text-[10px] pl-2 border-l-2 border-gray-200">
                                       <span className="text-gray-500 flex items-center gap-1"><Handshake className="w-3 h-3"/> チーム山分け分</span>
              ...
              {/* 📱 TAB 2: スタッフQR (Staff) */}
              {activeTab === 'qr' && (
                <div className="p-5 pt-10 flex flex-col items-center max-w-sm mx-auto h-full justify-center">
                  <div className="w-full bg-white p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center mb-6">
                    <div className="p-4 bg-white rounded-2xl shadow-sm border border-gray-100 mb-4">
                      <QRCodeCanvas value={referralUrl} size={180} level="H" fgColor="#111827" />
                    </div>
                    <p className="text-xs font-bold text-gray-600 text-center tracking-wider">お客様のスマートフォンで<br/>読み込んでください</p>
                  </div>
                  <button onClick={() => handleCopy(referralUrl)} className={`w-full py-3.5 rounded-xl font-semibold text-xs transition-all flex items-center justify-center gap-2 shadow-sm ${copied ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-700 hover:bg-gray-50'}`}>
                    {copied ? <CheckCircle2 className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                    {copied ? 'コピーしました！' : '接客用URLをコピー'}
                  </button>
                </div>
              )}
